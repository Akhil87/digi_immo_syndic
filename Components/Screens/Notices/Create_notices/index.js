...
const typeOptions = ["general", "specific", "building_specific"];

const Create_notice = ({ navigation, route }) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <BackButton navigation={navigation} title="Create Notice" />
      <ScrollView contentContainerStyle={styles.container}>
        {[
          ...
          const isEditable =
            !item ||
            ["name", "phoneNumber", "contactPerson", "email"].includes(key);
          return (
            <TextInput
              key={key}
        ...
        >
          <Text style={item && styles.disabledText}>
            {form.buildingId
              ? building?.find((b) => b.id === form.buildingId)?.buildingName ||
                "Select Building"
              : "Select Building"}
          </Text>
        </TouchableOpacity>
        ...
        >
          <Text style={styles.buttonText}>{item ? "Update" : "Create"}</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
